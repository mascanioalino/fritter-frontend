...
class FreetCollection {
  ...
  static async findOne(
    ...
    return FreetModel.findOne({ _id: freetId }).populate("authorId group");
  }
   ...
   * Get all the freets in by given author
   *
   * @param {string} groupName - The username of author of the freets
   * @return {Promise<HydratedDocument<Freet>[]>} - An array of all of the freets
   */
  static async findAllByGroupName(
    groupName: string
  ): Promise<Array<HydratedDocument<Freet>>> {
    const group = await GroupCollection.findOne(groupName);
    return FreetModel.find({ group })
      .sort({ dateModified: -1 })
      .populate("authorId group");
  }
